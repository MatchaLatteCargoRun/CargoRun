...
function normalize(row,columns,flightNumber,station){const get=names=>{const c=pick(columns,names);return c?row[c]:null},owner=station||{stationId:row.__StationId,stationCode:row.__StationCode,displayName:row.__StationDisplayName,timeZoneId:row.__TimeZoneId};let snapshot={};const raw=get(['SnapshotJson','Snapshot','DetailsJson']);if(raw){try{snapshot=typeof raw==='string'?JSON.parse(raw):raw}catch{}}return{id:String(get(['ImportCompletionRecordId','CompletionRecordId','Id'])??get(['VerificationId'])??''),flightId:get(['FlightId']),flight:flightNumber||snapshot.flight||'',flightDate:snapshot.flightDate||'',originAirport:snapshot.originAirport||'',destinationAirport:snapshot.destinationAirport||'',stationId:owner?.stationId?String(owner.stationId):null,stationCode:owner?.stationCode||null,displayName:owner?.displayName||null,timeZoneId:owner?.timeZoneId||null,finalizedBy:get(['FinalisedByDisplayName','FinalizedByDisplayName','FinalisedByName','FinalizedByName'])||snapshot.finalizedBy||'',finalizedById:get(['FinalisedByObjectId','FinalizedByObjectId','FinalisedById','FinalizedById'])||snapshot.finalizedById||'',finalizedAt:get(['FinalisedAtUtc','FinalizedAtUtc','FinalisedAt','FinalizedAt'])||snapshot.finalizedAt||null,verificationId:String(get(['VerificationId'])??snapshot.verificationId??''),recordHash:get(['RecordHash','Hash'])||'',exceptionReason:get(['ExceptionReason','CompletionReason','Reason'])||snapshot.exceptionReason||'',summary:snapshot.summary||{},ulds:Array.isArray(snapshot.ulds)?snapshot.ulds:[]}}
...
module.exports=async function(context,req){let pool,tx;try{
  ...
  if(req.method==='GET'){
    ...
    const r=await request.query(`SELECT i.*, f.FlightNumber AS __FlightNumber,
        CONVERT(varchar(20),f.StationId) AS __StationId,station.StationCode AS __StationCode,
        station.DisplayName AS __StationDisplayName,station.TimeZoneId AS __TimeZoneId
      FROM dbo.ImportCompletionRecords i
      INNER JOIN dbo.Flights f ON f.FlightId=i.${q(flightIdCol)}
      INNER JOIN dbo.CargoRunStations station ON station.StationId=f.StationId
  ...
  const auditRecord=normalize(inserted.recordset[0],columns,flight.FlightNumber,lockedAuthorization);
  ...
  const rec=normalize(inserted.recordset[0],columns,flight.FlightNumber,lockedAuthorization);rec.recordHash=hash;sendJson(context,201,{ok:true,record:rec,pendingCount});

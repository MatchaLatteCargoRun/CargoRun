-- READ ONLY. Detail extractor for STOP findings from multi-station-foundation-preflight.sql.
-- Result set 1: flight-level STOP details. Result set 2: orphan Offloads details.
SET NOCOUNT ON;
...
DECLARE @OffloadsObjectId int=OBJECT_ID(N'dbo.Offloads',N'U');
...
-- Result set 1: every flight behind an ownership or canonical-identity STOP finding.
...
BEGIN
  ...
  EvidenceRows AS (
    SELECT message.FlightId,message.MachMessageId,message.DocumentCorID,message.StationAirport,
    ...
    UNION ALL
    SELECT shipment.FlightId,shipment.MachMessageId,CAST(NULL AS nvarchar(200)),CAST(NULL AS nvarchar(20)),
  ...
  SELECT review.FlightId,review.FlightNumber,review.OperatingDate,review.Direction,
    ...
    review.OwnershipClassification AS Classification,review.ClassificationReason AS Reason,
    STUFF(CONCAT(
    ...
    ),1,1,N'''') AS FlightStopFindings,
    review.SourceIndicator,review.CreatedAtUtc,review.OriginalFlightNumber,review.CompactFlightNumber,
    review.NormalizedFlightNumber,review.SqlParityGuaranteed,review.CanonicalIdentityCollision,
    ...
    evidence.MachMessageId,evidence.DocumentCorID,evidence.StationAirport,evidence.SegmentOrigin,
...
ELSE
  SET @ExecutableSql=N'
    SELECT CAST(NULL AS bigint) AS FlightId,CAST(NULL AS nvarchar(100)) AS FlightNumber,
      ...
      CAST(NULL AS nvarchar(50)) AS FlightStatus,N''SCHEMA_UNAVAILABLE'' AS Classification,
      N''Flights core identity, route, or lifecycle columns are missing.'' AS Reason,
      N''MISSING_REQUIRED_SCHEMA'' AS FlightStopFindings,
...
-- Result set 2: every Offloads row behind the preflight ORPHAN_CHILD finding.
DECLARE @OffloadIdExpression nvarchar(300)=CASE
  WHEN COL_LENGTH(N'dbo.Offloads',N'OffloadId') IS NOT NULL THEN N'CONVERT(nvarchar(100),offload.OffloadId)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'Id') IS NOT NULL THEN N'CONVERT(nvarchar(100),offload.Id)'
  ELSE N'CAST(NULL AS nvarchar(100))' END;
DECLARE @OffloadUldIdExpression nvarchar(300)=CASE WHEN COL_LENGTH(N'dbo.Offloads',N'UldId') IS NOT NULL THEN N'CONVERT(nvarchar(100),offload.UldId)' ELSE N'CAST(NULL AS nvarchar(100))' END;
DECLARE @OffloadUldNumberExpression nvarchar(300)=CASE
  WHEN COL_LENGTH(N'dbo.Offloads',N'UldNumber') IS NOT NULL THEN N'CONVERT(nvarchar(100),offload.UldNumber)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'ULDNumber') IS NOT NULL THEN N'CONVERT(nvarchar(100),offload.ULDNumber)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'Uld') IS NOT NULL THEN N'CONVERT(nvarchar(100),offload.Uld)'
  ELSE N'CAST(NULL AS nvarchar(100))' END;
DECLARE @OffloadStatusExpression nvarchar(300)=CASE
  WHEN COL_LENGTH(N'dbo.Offloads',N'Status') IS NOT NULL THEN N'CONVERT(nvarchar(100),offload.Status)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'OffloadStatus') IS NOT NULL THEN N'CONVERT(nvarchar(100),offload.OffloadStatus)'
  ELSE N'CAST(NULL AS nvarchar(100))' END;
DECLARE @OffloadRequestedExpression nvarchar(300)=CASE
  WHEN COL_LENGTH(N'dbo.Offloads',N'RequestedAtUtc') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.RequestedAtUtc)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'RequestedAt') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.RequestedAt)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'CreatedAtUtc') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.CreatedAtUtc)'
  ELSE N'CAST(NULL AS datetime2(3))' END;
DECLARE @OffloadCollectedExpression nvarchar(300)=CASE
  WHEN COL_LENGTH(N'dbo.Offloads',N'CollectedAtUtc') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.CollectedAtUtc)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'CollectedAt') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.CollectedAt)'
  ELSE N'CAST(NULL AS datetime2(3))' END;
DECLARE @OffloadCompletedExpression nvarchar(300)=CASE
  WHEN COL_LENGTH(N'dbo.Offloads',N'DeliveredAtUtc') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.DeliveredAtUtc)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'DeliveredAt') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.DeliveredAt)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'CompletedAtUtc') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.CompletedAtUtc)'
  WHEN COL_LENGTH(N'dbo.Offloads',N'CompletedAt') IS NOT NULL THEN N'TRY_CONVERT(datetime2(3),offload.CompletedAt)'
  ELSE N'CAST(NULL AS datetime2(3))' END;
DECLARE @ParentFlightNumberExpression nvarchar(300)=CASE WHEN COL_LENGTH(N'dbo.Flights',N'FlightNumber') IS NOT NULL THEN N'CONVERT(nvarchar(100),flight.FlightNumber)' ELSE N'CAST(NULL AS nvarchar(100))' END;
DECLARE @ParentOperatingDateExpression nvarchar(300)=CASE WHEN COL_LENGTH(N'dbo.Flights',N'OperatingDate') IS NOT NULL THEN N'TRY_CONVERT(date,flight.OperatingDate)' ELSE N'CAST(NULL AS date)' END;
DECLARE @ParentDirectionExpression nvarchar(300)=CASE WHEN COL_LENGTH(N'dbo.Flights',N'Direction') IS NOT NULL THEN N'CONVERT(nvarchar(30),flight.Direction)' ELSE N'CAST(NULL AS nvarchar(30))' END;
DECLARE @ParentStatusExpression nvarchar(300)=CASE WHEN COL_LENGTH(N'dbo.Flights',N'FlightStatus') IS NOT NULL THEN N'CONVERT(nvarchar(100),flight.FlightStatus)' ELSE N'CAST(NULL AS nvarchar(100))' END;

IF @OffloadsObjectId IS NOT NULL AND @FlightsObjectId IS NOT NULL
   AND COL_LENGTH(N'dbo.Offloads',N'FlightId') IS NOT NULL
   AND COL_LENGTH(N'dbo.Flights',N'FlightId') IS NOT NULL
BEGIN
  SET @ExecutableSql=N'
    SELECT '+@OffloadIdExpression+N' AS OffloadId,CONVERT(nvarchar(100),offload.FlightId) AS FlightId,
      '+@OffloadUldIdExpression+N' AS UldId,'+@OffloadUldNumberExpression+N' AS UldNumber,
      '+@OffloadStatusExpression+N' AS Status,'+@OffloadRequestedExpression+N' AS RequestedAtUtc,
      '+@OffloadCollectedExpression+N' AS CollectedAtUtc,'+@OffloadCompletedExpression+N' AS CompletedAtUtc,
      '+@ParentFlightNumberExpression+N' AS FlightNumber,'+@ParentOperatingDateExpression+N' AS OperatingDate,
      '+@ParentDirectionExpression+N' AS Direction,'+@ParentStatusExpression+N' AS FlightStatus,
      CASE WHEN offload.FlightId IS NULL THEN N''NULL_FLIGHT_ID''
           WHEN flight.FlightId IS NULL THEN N''NONEXISTENT_FLIGHT_ID''
           ELSE N''INVALID_OWNERSHIP_LINK'' END AS OrphanReason
    ...
    ORDER BY CASE WHEN offload.FlightId IS NULL THEN 1 ELSE 2 END,offload.FlightId,OffloadId;';
END;
ELSE
  SET @ExecutableSql=N'
    SELECT CAST(NULL AS nvarchar(100)) AS OffloadId,CAST(NULL AS nvarchar(100)) AS FlightId,
      CAST(NULL AS nvarchar(100)) AS UldId,CAST(NULL AS nvarchar(100)) AS UldNumber,
      CAST(NULL AS nvarchar(100)) AS Status,CAST(NULL AS datetime2(3)) AS RequestedAtUtc,
      CAST(NULL AS datetime2(3)) AS CollectedAtUtc,CAST(NULL AS datetime2(3)) AS CompletedAtUtc,
      CAST(NULL AS nvarchar(100)) AS FlightNumber,CAST(NULL AS date) AS OperatingDate,
      CAST(NULL AS nvarchar(30)) AS Direction,CAST(NULL AS nvarchar(100)) AS FlightStatus,
      N''SCHEMA_UNAVAILABLE: Offloads.FlightId or Flights.FlightId is missing.'' AS OrphanReason;';
